...
1. [Requesting translations](1-requesting-translations.md): the three events
   that ask for a translation. Developer adds a label, culture is enabled,
   user hits a missing label.
2. [Inside "request missing translations"](2-request-missing-translations.md):
   the shared function all three call. Dedup, pending rows, chunked publish,
   hourly sweep.
...
```mermaid
sequenceDiagram
    actor People as Developer / User
    ...
    People->>App: add a label / enable a culture / hit a missing label
